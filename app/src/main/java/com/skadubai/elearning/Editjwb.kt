...
import com.skadubai.elearning.databinding.ActivityEditjwbBinding
...
class Editjwb : AppCompatActivity() {

    private lateinit var binding: ActivityEditjwbBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityEditjwbBinding.inflate(layoutInflater)
        setContentView(binding.root)

        ptNama = findViewById(R.id.txtnamapengirim) as EditText
        ptNama.setText(NamaPengirim)
        binding.titleFile.setText(Namajwb)
        ...
        if (requestCode == 111 && resultCode == RESULT_OK) {
            if ( resultCode.equals(-1)){
                ...
                binding.btnsubmit.setOnClickListener {
                    var alertDialog = AlertDialog.Builder(this@Editjwb)
                        .setTitle("Apakah Anda ingin Mengubah Jawaban?")
                        .setPositiveButton("Ya", DialogInterface.OnClickListener { dialog, which ->
                            RetrofitClient.instance.UpdateJwb(
                                Nama,
                                getName.toString(),
                                NISN,
                                Id_tugas,
                                Namajwb
                            ).enqueue(object  : Callback<UpdJwb>{
                                override fun onResponse(call: Call<UpdJwb>, response: Response<UpdJwb>) {
                                    Toast.makeText(this@Editjwb, "Sukses Edit Jawaban", Toast.LENGTH_SHORT).show()
                                    postImage(file, getName.toString())
                                    val submit = Intent(this@Editjwb, DetailTugas::class.java)
                                ...
                                override fun onFailure(call: Call<UpdJwb>, t: Throwable) {
                                    Log.d("ini kesalahan API jwb", NISN.toString())
                                }
...
//                            Toast.makeText(this@Jwbtugas, "Berhasil", Toast.LENGTH_SHORT).show()
                            ...
                            Toast.makeText(this@Editjwb, "salah", Toast.LENGTH_SHORT).show()
                        }

                    } catch (e: JSONException) {
                        e.printStackTrace()
                        Toast.makeText(this@Editjwb, "kesalahan", Toast.LENGTH_SHORT).show()
                    }
                }
